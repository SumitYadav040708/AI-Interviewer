...
function InterviewRunner() {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8 pb-32">
      <div className="flex justify-between items-center bg-slate-900 p-6 rounded-2xl shadow-sm border border-slate-800 mb-6">
        <div>
          <h1 className="text-xl font-black text-white">{activeSession.role}</h1>
          <div className="flex gap-2 mt-2">
            {activeSession?.questions?.map((q, i) => (
              <div
                key={i}
                onClick={() => handleNavigation(i)}
                className={`w-3 h-3 rounded-full cursor-pointer transition-all ${i === currentQuestionIndex ? 'bg-blue-500 scale-125 ring-2 ring-blue-300' :
                  q.isEvaluated ? 'bg-emerald-500' :
                    (q.isSubmitted || submittedLocal[i]) ? 'bg-amber-500 animate-pulse' : 'bg-slate-700'
                  }`}
              />
            ))}
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-slate-900 p-6 rounded-3xl border border-slate-800 shadow-sm flex flex-col items-center justify-center min-h-[300px]">
        ...
        <div className="bg-slate-900 p-2 rounded-3xl border border-slate-800 shadow-sm overflow-hidden h-[400px]">
          <div className="flex justify-between px-4 py-2 bg-slate-800 border-b border-slate-700">
            <span className="text-xs font-bold text-slate-300 uppercase py-2">Code Editor</span>
            <select
              value={selectedLanguage}
              onChange={(e) => setSelectedLanguage(e.target.value)}
              disabled={isQuestionLocked}
              className="text-xs bg-slate-900 border border-slate-700 rounded-lg px-2 text-white disabled:bg-slate-800 disabled:text-slate-400"
            >
      ...
      {currentQuestion?.isEvaluated && (
        <div className="mt-6 bg-slate-900 border border-slate-800 p-6 rounded-2xl animate-in fade-in slide-in-from-bottom-4">
          <h3 className="text-teal-400 font-bold mb-2">💡 AI Feedback</h3>
          <p className="text-slate-300 text-sm leading-relaxed">{currentQuestion.aiFeedback}</p>
          <div className="mt-4 flex gap-4">
            <span className="bg-slate-800 px-3 py-1 rounded-lg text-xs font-bold text-teal-300 shadow-sm">Score: {currentQuestion.technicalScore}/100</span>
          </div>
        </div>
      )}

      <div className="fixed bottom-0 left-0 right-0 bg-slate-900 border-t border-slate-800 p-4 px-6 md:px-12 flex justify-between items-center z-50">
        <button
          onClick={() => handleNavigation(currentQuestionIndex - 1)}
          disabled={currentQuestionIndex === 0}
          className="text-slate-400 font-bold text-sm hover:text-white disabled:opacity-30"
        >
          ← Previous
        </button>
        ...
        <div className="flex flex-col items-center">
          ...
          {isProcessing && message && (
            <div className="mb-2 text-xs font-mono text-teal-200 bg-slate-800 px-3 py-1 rounded-full animate-pulse border border-slate-700">
              🤖 {message}...
            </div>
          )}
