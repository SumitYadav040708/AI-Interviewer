...
function SessionReview() {
    ...
    if (!activeSession || activeSession.status !== 'completed') {
        return (
            <div className="max-w-xl mx-auto mt-10 sm:mt-20 p-6 sm:p-10 bg-slate-900 rounded-3xl sm:rounded-[2.5rem] shadow-2xl text-center border border-slate-800 ">
    ...
    return (
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8 sm:py-12 space-y-8 sm:space-y-12 animate-in fade-in duration-700">
            ...
            {/* --- Header --- */}
            <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4 border-b border-slate-700 pb-6 sm:pb-10">
                <div>
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 overflow-x-auto pb-4 sm:pb-0 no-scrollbar snap-x">
                {[
                    ...
                    <div key={i} className={`min-w-[160px] snap-center bg-slate-900 p-6 sm:p-8 rounded-3xl sm:rounded-[2.5rem] shadow-sm border-l-[8px] ${stat.color === 'teal' ? 'border-teal-500' : 'border-slate-700'}`}>
                        <p className="text-[9px] sm:text-[10px] font-black text-slate-400 uppercase tracking-[0.1em]">{stat.label}</p>
                        <p className={`text-2xl sm:text-4xl font-black mt-2 leading-none ${stat.color === 'teal' ? 'text-teal-600' : 'text-white'}`}>{stat.value}</p>
                    </div>
                ))}
            </div>

            {/* --- Chart --- */}
            <div className="bg-slate-900 p-6 sm:p-10 rounded-3xl sm:rounded-[3rem] shadow-sm border border-slate-700">
            ...
            <div className="space-y-6 sm:space-y-10">
                ...
                <div className="space-y-6 sm:space-y-10">
                    {questions.map((q, index) => (
                        <div key={index} className="bg-slate-900 rounded-3xl sm:rounded-[3rem] border border-slate-800 shadow-sm overflow-hidden group hover:shadow-lg transition-all duration-500">
                            <div className="p-6 sm:p-10 space-y-6 sm:space-y-8">
                                ...
                                <div className="flex flex-col lg:flex-row justify-between items-start gap-4 sm:gap-6">
                                    ...
                                    <div className="flex gap-2 shrink-0">
                                        <div className="px-3 py-1.5 sm:px-5 sm:py-2 rounded-xl sm:rounded-2xl border flex items-center gap-2 bg-slate-800 border-slate-700">
                                            <span className="text-[8px] sm:text-[10px] font-black uppercase text-slate-400">Tech</span>
                                            <span className="text-xs sm:text-sm font-black text-white">{q.technicalScore}%</span>
                                        </div>
                                        <div className="px-3 py-1.5 sm:px-5 sm:py-2 rounded-xl sm:rounded-2xl border border-slate-700 bg-slate-800/80 flex items-center gap-2">
                                            <span className="text-[8px] sm:text-[10px] font-black uppercase text-slate-400">Conf</span>
                                            <span className="text-xs sm:text-sm font-black text-white">{q.confidenceScore}%</span>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="space-y-3">
                                    ...
                                    <div className="bg-slate-800 rounded-2xl sm:rounded-[2rem] border border-slate-700 overflow-hidden">

                                        {/* Display Code if available */}
                                        {q.userSubmittedCode && q.userSubmittedCode !== "undefined" && (
                                            <div className="p-4 sm:p-6 border-b border-slate-700 last:border-0">
                                                <span className="text-[10px] font-bold text-slate-400 uppercase mb-2 block">Code</span>
                                                <pre className="text-[11px] sm:text-xs font-mono text-slate-200 whitespace-pre-wrap overflow-x-auto">
                                                    {q.userSubmittedCode}
                                                </pre>
                                            </div>
                                        )}
                                ...
                                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 sm:gap-10 pt-6 sm:pt-8 border-t border-slate-50">
                                    <div className="space-y-3">
                                        ...
                                        <div className="bg-slate-800/80 p-4 sm:p-6 rounded-2xl sm:rounded-[2rem] text-xs sm:text-sm italic text-slate-300 border-l-[4px] sm:border-l-[6px] border-teal-500 leading-relaxed">
                                            "{q.aiFeedback}"
                                        </div>
                                    </div>
                                    <div className="space-y-3">
