...
const inputBase = 'w-full bg-slate-800 border-2 border-slate-700 rounded-xl sm:rounded-2xl p-3.5 sm-4 font-semibold text-white text-base transition-all focus:bg-slate-900 focus:border-teal-500 outline-none';
const Profile = () => {
  ...
  return (
    <div className='max-w-4xl mx-auto px-4 py-6 sm:py-12 pb-24'>
      <div className='bg-slate-900 rounded-3xl shadow-xl sm:shadow-2xl p-6 sm:p-12 border border-slate-800'>
        ...
        <form onSubmit={handleSubmit} className='space-y-6' >
          ...
          <FormField label="Email Address (Fixed)" muted>
            <input
              type="email"
              className='w-full bg-slate-800 rounded-xl sm:rounded-2xl p-3.5 sm-4 font-semibold text-slate-300 text-base cursor-not-allowed border border-slate-700'
              disabled
          ...
          <div className='pt-4'>
            <button
              ...
              className={`w-full flex items-center justify-center gap-2 py-4 font-bold rounded-xl sm:rounded-2xl transition-all active:scale-[0.98] ${isProfileLoading ? 'bg-slate-700 text-slate-400 cursor-wait' : 'bg-teal-600 text-white hover:bg-teal-700 shadow-xl'}`}>
              {
