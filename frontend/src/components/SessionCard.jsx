const SessionCard = ({ session, onClick, onDelete }) => {
    ...
    const statusColor = session.status === 'completed' ? 'bg-emerald-900/70 text-emerald-200' : session.status === 'in-progress' ? 'bg-amber-900/60 text-amber-200' : 'bg-blue-900/60 text-blue-200';

    const iconBg = session.status === 'completed' ? 'bg-emerald-900/70 text-emerald-300' : 'bg-blue-900/70 text-blue-300';

    const scoreColor = session.status === 'completed' ? (session.overallScore > 75 ? ' text-emerald-300' : 'text-orange-300') : 'text-slate-300';

    return (
        <div onClick={() => onClick(session)} className='group bg-slate-900 border border-slate-800 p-5 sm:p-6 rounded-2xl sm:rounded-[2rem] flex flex-col md:flex-row items-center gap-4 transition-all hover:shadow-lg active:scale-[0.98] cursor-pointer'>
            <div className='flex items-center gap-4 sm:gap-6 w-full md:w-auto flex-grow'>
                ...
                <div className='overflow-hidden'>
                    <h3 className='font-bold text-white text-base sm:text-lg truncate group-hover:text-teal-600'>{session.role}</h3>
                    <div className='flex items-center gap-2 text-[10px] font-bold text-slate-400 mt-1 uppercase tracking-tight'>
                        <span>{new Date(session.createdAt).toLocaleDateString()}</span>
                        <span>.</span>
                        <span className='text-slate-200 bg-slate-800 px-1.5 py-0.5 rounded-md'>{session.level}</span>
                    </div>
                </div>
            </div>
            ...
            <div className='flex items-center justify-between md:justify-end gap-6 w-full md:w-auto border-t md:border-t-0 pt-3 md:pt-0'>
                ...
                <div className='flex flex-col items-end gap-1.5'>
                    ...
                    <span className='text-teal-400 font-bold text-xs flex items-center'>{session.status === 'completed' ? 'Results' : 'Resume'}
